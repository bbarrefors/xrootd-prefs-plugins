...
usage = "usage: IPGeoPlugin.py <ipaddress1> <ipaddress2>"
...
def main(argv):
    if len(argv) == 2:
        ...
        # Extract IP address from format [::IP]:PORT
        IP1 = str(argv[0]).strip('[::').partition(']')
        IP2 = str(argv[1]).strip('[::').partition(']')
        subnet1 = IPGeoPlugin.IPToSubnet(IP1[0])
        subnet2 = IPGeoPlugin.IPToSubnet(IP2[0])
        print subnet1, subnet2
        # Initialize SQLite3 db to check if IP is in cache
        # To implement: Clear old entries in cache (build another script)
        con = lite.connect('ip_cache.db')
        with con:
            cur = con.cursor()
            cur.execute('CREATE TABLE IF NOT EXISTS IPtoCoord (IP TEXT, Lat TEXT, Long TEXT)')
        distance = IPGeoPlugin.IPDistance(subnet1, subnet2, con)
        print distance
        sys.exit()
    else:
        print usage
